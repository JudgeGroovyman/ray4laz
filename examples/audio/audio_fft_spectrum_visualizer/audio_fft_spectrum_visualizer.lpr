...
begin
  ...
  fft.spectrum := GetMem(SizeOf(TFFTComplex) * FFT_WINDOW_SIZE);
  fft.workBuffer := GetMem(SizeOf(TFFTComplex) * FFT_WINDOW_SIZE);
  fft.prevMagnitudes := GetMem(SizeOf(Single) * BUFFER_SIZE);
